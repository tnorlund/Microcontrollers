...
bool readBlock( uint32_t blockaddr, uint8_t *buffer ) {
    uint32_t ii;      // iterator for counting 512 BYTES
    uint8_t reply;    // Space that catches falling edge of sendCommand
    uint8_t response; // Response from the slave
    uint8_t garbage;  // The two extra bytes used as a stop

  // Adjust the address based on the type of card
  blockaddr <<= blockScaling;

  reply = sendCommand(
          CMD17,               // command being send
          blockaddr,           // location on SD Card
          (uint8_t *)&response // place to store response
  );

  if (reply & 0x01) { // check the reply a failed read
      return false;
  } else {            // reply confirms read
      while(rxByteSSI0() != 0xFE);  // wait for falling edge

      for(ii=0; ii<512; ii++) {     // read 512 BYTE into buffer
          buffer[ii] = rxByteSSI0();
      }

      garbage = rxByteSSI0(); // remove remaining 2 STOP BYTES
      garbage = rxByteSSI0();
